...
void TscWrite(uint8_t ucReg, uint8_t ucNum, uint32_t ulVal)
{
  ...
  SSPBUF = (unsigned char)(ucReg & 0x7F);        // register address and WRITE operation
  dummy = SSPBUF;
...
void TscRead(uint8_t ucReg, uint8_t ucNum, uint8_t* pucBuffer)
{
  //SSPCON1 = 0b00110010;     // SSPEN & CKP & speed select (fosc/64) 1MHz
  nCS_TSC = 0;                    // chip select
  SSPBUF = (unsigned char)(ucReg | 0x80);          // register address READ OPERATION
  dummy = SSPBUF;
  while(SSPIF==0){};
  SSPIF = 0;
  (void)SSPBUF;
...
void TscUpdate(void)
{
  ...
  if((intStat & 0x02)||(tempTbl[0] == 0x10))    // data in fifo ?
  {
    ...
    yPos = ((uint16_t)tempTbl[0] << 8) | tempTbl[1];
#endif
    tsc.posY = (uint16_t)(((yPos *(LCD_HEIGHT-1)) / (MAX_Y_MEAS-MIN_Y_MEAS))-(((LCD_HEIGHT-1) * MIN_Y_MEAS) / (MAX_Y_MEAS-MIN_Y_MEAS)));
    tsc.posX = (uint16_t)(((xPos *(LCD_WIDTH-1)) / (MAX_X_MEAS-MIN_X_MEAS))-(((LCD_WIDTH-1) * MIN_X_MEAS) / (MAX_X_MEAS-MIN_X_MEAS)));
//    tsc.posX = xPos;
//    tsc.posY = yPos;
  }
